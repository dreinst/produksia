...
import { laporanPiutang, KERANJANG_UMUR } from "@/lib/laporanRekanan";
...
export default async function HalamanPelangganUmur({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const laporan = await laporanPiutang(db, periode.sampai);
  ...
  return (
    <div className="space-y-6">
      <KepalaHalaman
        jejak={[{ label: "Laporan" }]}
        judul="Laporan Piutang Usaha"
        subjudul={`Per ${tanggal(periode.sampaiTeks)} — Sisa tagihan pelanggan per tanggal, dikelompokkan menurut umur terhadap jatuh tempo (TOP). Faktur dikurangi uang muka, penerimaan, dan retur s.d. tanggal laporan.`}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
        <div className="kartu p-4"><div className="teks-label">Total sisa</div><div className="font-heading text-lg font-bold angka mt-1">{rp(laporan.total)}</div><div className="text-xs text-slate-500">{laporan.jumlahFaktur} faktur · {laporan.kelompok.length} Pelanggan</div></div>
      ...
      <TabelUmur laporan={laporan} labelRekanan="Pelanggan" tautanFaktur="/penjualan/faktur" />
    </div>
  );
}
