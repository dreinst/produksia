...
export default async function CashOutPage() {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-xl font-semibold">Kas Keluar</h1>

      <ActionForm action={createCashOutForm} className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl border rounded-lg p-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium">Akun Lawan (tujuan/beban) *</label>
      ...
      <div className="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0">
        ...
        <thead>
          <tr className="border-b text-left">
            ...
            <th className="py-2 pr-4">Ke Akun</th>
            <th className="py-2 pr-4 text-right">Jumlah</th>
          </tr>
        </thead>
        <tbody>
          {entries.map((e) => {
            const cashLine = e.lines.find((l) => Number(l.credit) > 0);
            const counterLine = e.lines.find((l) => Number(l.debit) > 0);
            return (
              <tr key={e.id} className="border-b">
                ...
                <td className="py-2 pr-4 text-right">
                  {Number(cashLine?.credit ?? 0).toLocaleString("id-ID")}
                </td>
              </tr>
            );
          })}
