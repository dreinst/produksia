...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const foto = await db.foto.findUnique({ where: { id }, select: { id: true, tipe: true, ukuran: true, isi: true, barangId: true, peminjamanId: true } });
  if (!foto) return Response.json({ ok: false, galat: "Foto tidak ditemukan" }, { status: 404 });

  const hak = foto.barangId ? "persediaan.lihat" : "peminjaman.lihat";
