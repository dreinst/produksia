...
export default function ReceiptLinesPicker({ lines }: { lines: OrderLine[] }) {
  const initial = lines.map((l) => ({
    orderLineId: l.id,
    itemId: l.itemId,
    qty: Math.max(Number(l.qty) - Number(l.qtyReceived), 0),
  }));
      ...
      <div className="overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0">
        ...
        <thead>
          <tr className="text-left text-zinc-500">
            ...
            <th className="pb-1 w-28">Qty Terima</th>
          </tr>
        </thead>
        <tbody>
          {lines.map((l, i) => {
            const remaining = Number(l.qty) - Number(l.qtyReceived);
            return (
              <tr key={l.id}>
