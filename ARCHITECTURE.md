...
Produksia adalah aplikasi web internal untuk usaha event/wedding organizer yang mengikuti **alur kerja** software akuntansi pada umumnya. Satu proses Next.js melayani UI sekaligus logika bisnis (server aksi), dengan PostgreSQL sebagai satu-satunya sumber kebenaran.
...
### 6.2h Dimensi proyek/event, LPJ, dan laporan tambahan
`Proyek` menyimpan nilai kontrak, anggaran biaya, tanggal; `proyekId` ada di Penawaran, Pesanan Penjualan, Pesanan Pembelian, dan **Jurnal**. `bacaProyekId` (`src/lib/proyek.ts`) membaca pilihan formulir; `tandaiProyek` (`akuntansi.ts`) dipanggil setiap aksi dokumen setelah jurnal otomatisnya dibuat (SJ/FJ/UM/TRM/RJ dari pesanan penjualan, TB/FB/BYR/RB dari pesanan pembelian); jurnal manual & kas menyimpan `proyekId` langsung. `saldoAkunPeriode(..., { proyekId })` → `hitungLabaRugi` per event; `hitungLabaRugiBulanan` mengelompokkan baris jurnal per bulan. `ringkasanProyek` menyusun LPJ (pesanan, DP, faktur & TOP, penerimaan, PSB/FB/BYR, beban lewat kas/jurnal, laba/rugi & kas dari jurnal bertanda event). `laporanRekanan.ts`: umur piutang/hutang per tanggal (5 keranjang) dan perubahan modal (ekuitas awal + setoran + laba − prive ± lainnya = ekuitas akhir, dicek ke neraca). `laporanPendapatan.ts`: Ringkasan Pendapatan periode (`/laporan/pendapatan`) dari baris jurnal akun PENDAPATAN tanpa jurnal penutup; per pelanggan lewat `jurnal.fakturPenjualan`/`returPenjualan` (baris tanpa keduanya, mis. kas masuk atau jurnal umum, jadi satu baris "tanpa pelanggan"), per jenis layanan memakai `susunHierarki` yang sama dengan Laba Rugi sehingga totalnya selalu sama. `basisKas.ts` (`labaRugiKas`, `labaRugiKasBulanan`): sudut pandang kas untuk Laba Rugi (bawaan `basis=kas`), Ringkasan Pendapatan, dan tren beranda; membaca jurnal yang menyentuh kas/bank seperti Arus Kas, mengalokasikan kas dari pelanggan ke akun pendapatan menurut baris faktur/pesanan (PPN dipisah sebagai titipan), mengelompokkan kas operasi lain per akun lawan; surplusnya = arus kas operasi. `pajak.ts` membaca omzet bruto usaha dari buku besar (akun pendapatan minus kelompok `diskonPenjualan` dan `pendapatanLain` di `PemetaanAkun`). `verifikasi.ts` + `<FormulirAksi verifikasi>` + `DialogVerifikasi`: dialog ringkasan isian & peringatan sebelum simpan. `flagshipStandar.ts`: data induk tiket/sponsor/booth + Pelanggan Umum, diterapkan bersama bagan akun. Kebijakan lengkap di `KEBIJAKAN-AKUNTANSI.md`. `sdm.ts` + `aksi/sdm.ts`: Siklus SDM/Penggajian — `Karyawan` diperluas (jabatan, status, gajiPokok, tunjangan, akunBebanId opsional); `Penggajian`/`BarisPenggajian` satu dokumen per periode (bulan) mencakup beberapa karyawan; akun bawaan (Beban Gaji Pokok, Beban Tunjangan, Hutang Potongan Gaji) lewat `PemetaanAkunTambahan` (kunci `bebanGaji`/`bebanTunjangan`/`hutangPotonganGaji`) dengan fallback ke akun standar 5-2100/5-2500/2-1310; jurnal `PENGGAJIAN` dibangun manual (seperti Prive/PPh Final) karena butuh tanggal & sumber sendiri, bukan lewat `catatJurnal` generik. Tidak ada kode tambahan di laporan: karena jurnalnya menyentuh akun Beban dan Kas/Bank biasa, otomatis terhitung di Laba Rugi, Arus Kas, dan Laba Rugi Basis Kas.

### 6.2i Impor mutasi & rekonsiliasi kas/bank (`src/lib/mutasiBank.ts`, `src/lib/aksi/rekonsiliasi.ts`)
`bacaMutasi` membaca CSV/TSV (pembatas otomatis, kutip) atau HTML (tabel dengan baris terbanyak), memetakan tajuk ke kolom (`petakanKolom`, bisa ditimpa manual), membaca tanggal & angka berbagai format, dan mengabaikan baris yang tidak terbaca dengan alasan. Kolom Debit/Kredit dibaca menurut **sudut pandang**: rekening koran (Kredit bank = uang masuk) atau sudut buku (Debit = uang masuk), dideteksi dari pergerakan kolom Saldo bila tidak dipaksa; hasilnya selalu disimpan dari sudut buku (`masuk` = Dr akun kas/bank, `keluar` = Cr). `imporMutasi` menyimpan `MutasiBank` per akun kas/bank dengan `sidik` (hash akun+tanggal+nominal+keterangan) unik → impor ulang tidak menggandakan. `cocokkanOtomatis` memasangkan mutasi ↔ `BarisJurnal` akun yang sama (nominal & arah sama, tanggal terdekat dalam toleransi) dan mengisi `rekonsiliasiPada`; `cocokkanManual`/`lepasCocok`/`tandaiBarisCocok` untuk sisanya. Halaman rekonsiliasi menghitung saldo buku, selisih waktu (di buku belum di rekening / sebaliknya), saldo rekening seharusnya, dan membandingkannya dengan saldo rekening koran terakhir yang diimpor.

### 6.2j Prive, pemetaan akun tambahan, dan nego harga
`buatPrive` (`src/lib/aksi/prive.ts`) mencatat pengambilan pribadi pemilik: jurnal `PRV` sumber `PRIVE` (Dr akun prive [MODAL] / Cr kas-bank), tabel `Prive` (nomor `PRV`, pemilik, akun, jumlah), bisa dihapus lewat `hapusDokumen("prive")`. `hitungLaporanPrive` (`src/lib/laporanPrive.ts`) mengelompokkan per pemilik untuk Laporan Prive. Akun prive bawaan dibaca dari **pemetaan akun tambahan** (`PemetaanAkunTambahan`: `kunci` slug dari label, `akunId`), yang bisa ditambah/diganti/dihapus di Pengaturan → Pemetaan Akun oleh pemegang `pemetaan.tulis` (Admin ke atas); modul lain membaca dengan `akunPemetaanTambahan(db, kunci)`.
Harga: `Barang.hargaJual` (harga jual/markup) dan `Barang.hargaMinimum` (batas nego, 0 = tanpa batas). `periksaHargaNego` di `src/lib/aksi/penjualan.ts` (dipanggil `buatPenawaran` dan `buatPesanan`) menolak harga di bawah harga jual bila pengguna tidak punya `harga.nego`, dan di bawah harga minimum bila bukan Pemilik/Superadmin; `EditorBarisBarang` mengunci isian harga tanpa hak dan menampilkan petunjuk batas nego. Stok per Gudang menampilkan harga jual dan margin (Rp, %) di kanan harga pokok.
Rekonsiliasi kas/bank: `cocokkanOtomatis` menandai `MutasiBank.perluPerhatian` bila tanggal mutasi dan tanggal jurnal berbeda; halaman menampilkan kotak centang hanya untuk mutasi tersebut, `konfirmasiPerhatian` menyimpan `dikonfirmasiPada`; cocok manual dianggap sudah dicek; lepas cocok menghapus tanda.
...
- **Hak per dokumen** (`hakAkses.ts`, boleh diimpor komponen client): `DOKUMEN_HAK` mendaftar 22 jenis dokumen dengan aksi `lihat`/`buat`/`hapus` (mis. `faktur.buat`, `pengiriman.hapus`), ditambah 13 hak modul (`data-induk.lihat|tulis`, `persediaan.lihat`, `buku-besar.lihat|tulis`, `pengaturan.tulis`, `pengguna.kelola`, `log-aktivitas.lihat`, `rekonsiliasi.lihat|tulis`, `harga.nego`, `pemetaan.tulis`, `hak-akses.kelola`). `HAK_BAWAAN` per peran: Superadmin (admin IT) & Pemilik penuh, setara, dan tak bisa dikurangi (`PERAN_TERTINGGI`); Admin = semua dokumen, laporan, rekonsiliasi, data induk, log — tanpa `pengaturan.tulis`, `pengguna.kelola`, `buku-besar.tulis`, `hak-akses.kelola` (batasnya diatur Pemilik); Kasir = dokumen penjualan/pembelian/kas (lihat & buat) + data induk, tanpa laporan; Gudang = SJ/TB/stok. **Penyesuaian** disimpan di tabel `HakAksesPeran` (peran, hak, boleh) hanya sebagai selisih dari bawaan; `penggunaSaatIni` menghitung `hak` efektif tiap permintaan (`hitungHak`) sehingga perubahan di Pengaturan › Hak Akses langsung berlaku. `punyaHak(sesi, hak)` memakai hak efektif; `punyaHak(peran, hak)` memakai bawaan. Menghapus dokumen memakai `HAK_HAPUS[jenis]` (jurnal menurut sumbernya). Sidebar, menu Transaksi Baru, dan tombol aksi di daftar disaring dengan `punyaHak` — itu kenyamanan, bukan pengaman.
...
- **Lupa kata sandi** (`PermintaanAturUlang`): `/lupa-kata-sandi` (publik) membuat permintaan MENUNGGU tanpa membocorkan keberadaan akun dan tanpa menggandakan permintaan terbuka; `buatTautanAturUlang` (hak `pengguna.kelola`, akun tingkat tertinggi hanya oleh tingkat tertinggi) membuat token acak 24 byte + kedaluwarsa 24 jam (status TAUTAN) yang ditampilkan di halaman Pengguna untuk diserahkan langsung; `/atur-ulang/[token]` (publik) memverifikasi token lalu `pakaiTautanAturUlang` mengganti hash, menghapus token (SELESAI), mencabut semua sesi, dan membuat sesi baru. `tolakPermintaanAturUlang` → DITOLAK; atur ulang manual oleh admin menutup permintaan terbuka. Semua tercatat di `LogAktivitas`. `proxy.ts` mengecualikan `/lupa-kata-sandi` dan `/atur-ulang`.
