...
`Proyek` menyimpan nilai kontrak, anggaran biaya, tanggal; `proyekId` ada di Penawaran, Pesanan Penjualan, Pesanan Pembelian, dan **Jurnal**. `bacaProyekId` (`src/lib/proyek.ts`) membaca pilihan formulir; `tandaiProyek` (`akuntansi.ts`) dipanggil setiap aksi dokumen setelah jurnal otomatisnya dibuat (SJ/FJ/UM/TRM/RJ dari pesanan penjualan, TB/FB/BYR/RB dari pesanan pembelian); jurnal manual & kas menyimpan `proyekId` langsung. `saldoAkunPeriode(..., { proyekId })` → `hitungLabaRugi` per event; `hitungLabaRugiBulanan` mengelompokkan baris jurnal per bulan. `ringkasanProyek` menyusun LPJ (pesanan, DP, faktur & TOP, penerimaan, PSB/FB/BYR, beban lewat kas/jurnal, laba/rugi & kas dari jurnal bertanda event). `laporanRekanan.ts`: umur piutang/hutang per tanggal (5 keranjang) dan perubahan modal (ekuitas awal + setoran + laba − prive ± lainnya = ekuitas akhir, dicek ke neraca).
