...
10. **[FIXED] Tidak ada penanda menu aktif; 31 link ditampilkan sekaligus.** Sidebar sekarang accordion 7 menu utama (Penjualan, Pembelian, Kas & Bank, Buku Besar, Aset Tetap, Persediaan, Daftar) + Beranda; sub-menu hanya muncul saat menu utamanya diklik, dan menu yang memuat halaman aktif terbuka otomatis. Halaman aktif disorot (`aria-current="page"`), tombol accordion punya `aria-expanded`/`aria-controls`.
...
15. **[FIXED] Nama prop menyesatkan** `sellPrice` di `LineItemsEditor` (diisi `costPrice` untuk PSB) → `defaultPrice`.
